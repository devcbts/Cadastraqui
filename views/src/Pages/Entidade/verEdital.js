...
export default function VerEditalEntidade() {
  ...
  async function getApplications() {
    ...
    try {
      const response = await api.get(
        `/entities/applications/${announcement_id.announcement_id}`,
        {
          headers: {
            authorization: `Bearer ${token}`,
          },
  ...
  useEffect(() => {
    async function fetchAnnouncements() {
      ...
      try {
        const response = await api.get(
          `/entities/announcement/${announcement_id.announcement_id}`,
          {
            headers: {
              authorization: `Bearer ${token}`,
            },
    ...
    const findEntityOrSubsidiaryName = (level) => {
      ...
      if (level.entitySubsidiaryId) {
        const subsidiary = subsidiaries.find(
          (sub) => sub.id === level.entitySubsidiaryId
        );
    ...
    return (
      <div key={educationLevel.id} className="education-level-details">
        ...
        <div className="course-details">
          <h1>
            Matriz ou Filial: {findEntityOrSubsidiaryName(educationLevel)}
          </h1>
          <h1>
            Curso: {educationLevel.availableCourses || educationLevel.grade}
          </h1>
          ...
          {!educationLevel.basicEduType ? (
            ...
            <div>
              <h2>
                Tipo de bolsa:{" "}
                {translateBasicEducationScholashipofferType(
                  educationLevel.scholarshipType
                )}
              </h2>
              <h2>
                Grau de Escolaridade:{" "}
                {translateBasicEducationScholashipType(
                  educationLevel.basicEduType
                )}
              </h2>
            </div>
      ...
      <div className="container-open-edital">
        ...
        <div className="container-inscricao">
          <div
            ...
            style={{ height: "fit-content", overflowX: "clip" }}
          >
            {profilePhoto ? (
...
const ScholarshipOfferType = [
  ...
  {
    value: "StudentWithDisabilityPartial",
    label: "Estudante com Deficiência Parcial",
  },
  {
    value: "StudentWithDisability",
    label: "Estudante com Deficiência Integral",
  },
