...
export default function IncomeList({ applicationId, onSelect }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true)
            try {
                const members = await applicationService.getAllIncomes(applicationId)
                if (members) {
                    setMembers(members)
                }
            } catch (err) {
    ...
    return (
        <>
            ...
            {
                selectedMember &&
                <MemberIncomeView member={selectedMember} onSelect={(member) => onSelect(member)} applicationId={applicationId} onBack={() => setSelectedMember(null)} />
            }
        </>
    )
}