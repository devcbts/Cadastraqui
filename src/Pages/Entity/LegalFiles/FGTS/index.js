...
export default function FGTS() {
    const { documents, handleUploadFile } = useLegalFiles({ type: 'FGTS' })
    ...
    return (
        <>
            ...
            <div style={{ width: 'max(280px,60%)', display: 'flex', margin: 'auto', flexDirection: 'column', alignItems: 'self-start' }}>
                <InputForm control={control} name={'issuedAt'} label={'Certificado emitida em'} type="date" />
                <InputForm control={control} name={'expireAt'} label={'Certificado válido até'} type="date" />
