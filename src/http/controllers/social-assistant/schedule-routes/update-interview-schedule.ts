...
export default async function updateInterviewSchedule(request: FastifyRequest,
    ...
    try {
        ...
        if (new Date(beginHour) < schedule.announcement.interview.beginHour || new Date(endHour) > schedule.announcement.interview.endHour) {
        ...
        return reply.status(201).send({ message: "Agenda de entrevista atualizada com sucesso" })
