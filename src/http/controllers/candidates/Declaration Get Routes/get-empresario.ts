...
export default async function getEmpresario(
  ...
  try {
    ...
    const incomeDetails = await prisma.familyMemberIncome.findFirst({
        where: {OR: [{candidate_id: _id}, {legalResponsibleId: _id}], employmentType: 'BusinessOwner'},
    })
    if (!incomeDetails) {
    ...
    if (!infoDetails) {
        ...
        const incomeDetails = await prisma.familyMemberIncome.findFirst({
            where: {familyMember_id: _id, employmentType: 'BusinessOwner'},
        })
        if (!incomeDetails) {
            throw new ResourceNotFoundError()
        }
        return reply.status(200).send({...infoDetails, averageIncome: incomeDetails.averageIncome, CNPJ: incomeDetails.CNPJ, socialReason: incomeDetails.socialReason})
