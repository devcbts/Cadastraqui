...
export default async function getRuralWorker(
  ...
  try {
    ...
    const incomeDetails = await prisma.familyMemberIncome.findFirst({
        where: {OR: [{candidate_id: _id}, {legalResponsibleId: _id}], employmentType: 'TemporaryRuralEmployee'},
    })
    if (!incomeDetails) {
    ...
    if (!infoDetails) {
        ...
        const incomeDetails = await prisma.familyMemberIncome.findFirst({
            where: {familyMember_id: _id, employmentType: 'TemporaryRuralEmployee'},
        })
        if (!incomeDetails) {
            throw new ResourceNotFoundError()
        }
